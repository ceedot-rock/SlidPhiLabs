...
async function loadApiHandler(apiPath) {
  // apiPath like /api/welcome or /api/phi/metrics
  let rel = apiPath.replace(/^\/api\/?/, "");
  if (!rel) return null;
  // try file.js, file/index.js
  const candidates = [
...
function resolveStatic(pathname) {
  ...
  // cleanUrls: /foo → /foo.html if exists
...
const server = http.createServer(async (req, res) => {
  try {
      ...
      const body = attachNca(
        {
          ok: true,
          host: "fly",
          app: process.env.FLY_APP_NAME || "slidphilabs",
    ...
    // NCA infra coat — process reporting surface
